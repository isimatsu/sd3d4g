<?php
    ...
    if (isset($_POST['memo_add'])) {
        ...
        try {
            $memo_sql = "UPDATE trip_info SET memo = :memo WHERE segment_id = :segment_id";
            $stmt = $pdo->prepare($memo_sql);
            $stmt->bindValue(':memo', $memo, PDO::PARAM_STR);
            $stmt->bindValue(':segment_id', $sql_segment_id, PDO::PARAM_INT);
            $stmt->execute();

            header("Location: " . $_SERVER['REQUEST_URI']);
            exit();
        } catch (PDOException $e) {
            echo "メモの保存に失敗しました: " . htmlspecialchars($e->getMessage());
        }
    }
?>
<!DOCTYPE html>
...
<body>
    <main>
        <section class="sm" style="position: relative;">
            <div class="header">
            ...
            <div class="page-contents">
                ...
                <div class="planpage-music-list">
                    ...
                    <?php
                        $music = []; // 初期化
                        try {
                            // trip_song_connect と song2 を結合してデータを取得
                            $sql = "SELECT s2.* FROM `trip_song_connect` AS tsc 
                                    INNER JOIN `song2` AS s2 ON tsc.song_id = s2.song_id 
                                    WHERE tsc.trip_id = ?";
                            $stmt = $pdo->prepare($sql);
                            $stmt->execute([$plan_id]);
                            ...
                            if (empty($music)) {
                                echo '<p style="padding: 10px; color: #666;">この旅程に関連付けられた楽曲は見つかりませんでした。</p>';
                            } else {
                            ...
                            echo '<p style="color:red; padding:10px;">楽曲データの取得中にエラーが発生しました: ' . htmlspecialchars($e->getMessage()) . '</p>';
                            // JSエラーを防ぐために空配列を保証
                            $music = [];
                        }
                    ?>
...
if (isset($_POST['edit_segment_id'])) {
    ...
    <div class='modal-outline memo-modal' id='modal_outline'>
        <div class='modal-area'>
            ...
            <form action="index.php?plan_id=<?= htmlspecialchars($plan_id) ?>" method="POST">
                <input type="text" name="memo_add" class="memo-add-form" placeholder="メモを入力">
                <input type="hidden" name="segment_id" value="<?= htmlspecialchars($edit_segment_id) ?>">
